...
df_new_clients[indices, ] %>% 
  dplyr::select(emp.var.rate, cons.price.idx, cons.conf.idx, euribor3m) %>% 
  head(20) %>% 
  xtable(
...
```{r}
results_pca_df_client['marital'] <- df_new_clients['marital']
results_pca_df_client['job'] <- df_new_clients['job']
results_pca_df_client['education'] <- df_new_clients['education']
```

\begin{figure}
```{r fig.height=8, fig.width=5}
plot_marital <- results_pca_df_client %>% 
  ggplot(aes(x = PC1, y = PC2, color = marital)) +
  geom_point(size = 1)

plot_job <- results_pca_df_client %>% 
  ggplot(aes(x = PC1, y = PC2, color = job)) +
  geom_point(size = 1)

plot_education <- results_pca_df_client %>% 
...
During the testing it turned out that our set up from first project was wrong because R treated \textit{no} in target variables as \textit{1} and because of that our results was so superior. In the following experiments we've fixed this bug.

```{r}
measures = list(
  msr("classif.precision", id = "prec_tr", predict_sets = "train"),
  msr("classif.precision", id = "prec_tst"),
  msr("classif.recall", id = "recall_tr", predict_sets = "train"),
  msr("classif.recall", id = "recall_tst"),
  msr("classif.prauc", id = "aucpr_tr", predict_sets = "train"),
  msr("classif.prauc", id = "aucpr_tst")
)
...
results_balancing %>% 
  ...
  arrange(desc(aucpr_test)) %>% 
  xtable(
    caption = "Results of the classification accuracy of the models trained on dataset with and without PCA transformations with usage of undersampling and oversampling. Please note that \textit{ranger} in \textit{learner_id} is alias for Random Forest model with 100 trees.",
...
In order to better understand our results we've plotted them in 2D PCA space on Figure \ref{fig:kmeans-pca-space}. We can observe some distinct groups even in this 2D visualization which fill us with hope that obtained results will be interpretable.
...
fviz_silhouette(sil.pam13, xlab="PAM")
```
...
table(pam.k13$clustering, df_new_clients_sample$y) %>% 
  xtable(
    caption = "Clusters vs. target variable",
    label = "tab:pam-cluster-vs-target"
  )
```

First of all, this time we also could find any structure in terms of target variable as we can observe in Table \ref{tab:pam-cluster-vs-target}. Moreover we found that:
\begin{itemize}
...
```{r }
results_agnes_average <- agnes(dissimilarities_client, diss = TRUE, method = 'average')
results_agnes_single <- agnes(dissimilarities_client, diss = TRUE, method = 'single')
results_agnes_complete <- agnes(dissimilarities_client, diss = TRUE, method = 'complete')
```

```{r }
fig_average <- fviz_dend(results_agnes_average, cex=0.4, main="Dendrogram - average linkage")
fig_single <- fviz_dend(results_agnes_single, cex=0.4, main="Dendrogram - single linkage")
fig_complete <- fviz_dend(results_agnes_complete, cex=0.4, main="Dendrogram - complete linkage")
```

\begin{figure}
```{r fig.width=5, fig.height=8}
fig_average / fig_single / fig_complete
```
\caption{Dendograms obtained for AGNES algorithm using average, single and complete linkage.}
\label{fig:agnes-dendograms}
\end{figure}

The results of the clustering are presented on dendograms on Figure \ref{fig:agnes-dendograms}. As we can observe the results of single linkage are very unclear and it would be very hard to extract some meaningful clusters. However complete linkage and average linkage are doing good job and we can recognize some groups. Let's explore in more details average linkage version of AGNES algorithm.
...
for (k in K.range){
  agnes.k <- cutree(results_agnes_average, k = k)
  sil.agnes <- silhouette(agnes.k, dissimilarities_client)
  agnes.silhouettes <- c(agnes.silhouettes, mean(sil.agnes[, 3]))
}
```
...
data.frame(
  K = K.range,
  Silhouette = agnes.silhouettes
) %>% 
  ggplot(aes(x = K, y = Silhouette)) +
    geom_line() + 
    geom_point() +
    scale_x_discrete(limits = K.range) + 
    ggtitle("Comparison of the average silhouette vs K for AGNES (average linkage) algorithm.")
```
\caption{Values of average silhouette width for different number of cluster K in AGNES (average linkage) algorithm.}
\label{fig:agnes-average-silhouette}
\end{figure}

On the Figure \ref{fig:agnes-average-silhouette} we present average silhouette depending on number of clusters. We can observe that it's from up K equal 3 it's nondecreasing up to around K equal 30. Because of that we selected K = 28 which is maxium in our selected range. Let's find out what values are inside corresponding clusters.

\begin{figure}
```{r fig.height=4, fig.width=8, results='hide'}
agnes.k28 <- cutree(results_agnes_average, k = 28)
sil.agnes28 <- silhouette(agnes.k28, dissimilarities_client)
fviz_silhouette(sil.agnes28, xlab="AGNES")
```
\caption{Clusters silhouette plot for AGNES algorithm with K = 28.}
\label{fig:agnes-silhouette}
\end{figure}

From Figure \ref{fig:agnes-silhouette} we can observe that most of the clusters has very high value of silhouette and only few of them has some negative values. The results look very interesting and now check how the clusters look inside.

```{r results='hide'}
table(agnes.k28, df_new_clients_sample$y)
```

As previously we weren't be able to find groups of clients which were mostly subscribing term deposits. However we can procede with exploration of obtained clusters. Based on cluster assignments and values for corresponding clients' variables we found out following things.
\begin{itemize}
  \item Cluster 8 represents clients with university degree, single, mostly working in administration, without house and loan.
  \item Cluster 10 represents clients with university degree, working in administration, with house and without loan.
  \item Cluster 11 represents clients with basic education, married, working as blue-collars, with loans.
  \item Other clusters have also interesting representations however provided ones were the most representative.
\end{itemize}
...
```{r results='hide'}
table(agnes.k28, df_new_clients_sample$education)
```
```{r results='hide'}
table(agnes.k28, df_new_clients_sample$marital)
```
```{r results='hide'}
table(agnes.k28, df_new_clients_sample$job)
```
```{r results='hide'}
table(agnes.k28, df_new_clients_sample$housing)
```
```{r results='hide'}
table(agnes.k28, df_new_clients_sample$loan)
```

During this section we've tested a few clustering algorihtm thanks to which we've able to better understand clients group in our dataset. We've found consistent groups of clients, splitted them into clearly separable groups and due to that we've gained better understanding of data structure. This information could be very beneficial for analysts for futher analysis process, e.g. clients segmentation / pricing or for stakeholders which by better understanding of their client base could take further actions related to the whole business.
